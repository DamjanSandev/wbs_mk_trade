...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--rolling-backtest",
        action="store_true",
        help="Run configured rolling cutoffs and random seeds (computationally expensive)",
    )
...
def _train_and_eval(
    ...
    try:
        ...
        logger.info(f"  Test AUC={test_metrics['roc_auc']:.4f} "
                   ...
                   f"NDCG@10={test_metrics.get('ndcg@10', 0.0):.4f}")
        return test_metrics
...
def _run_rolling_backtests(
    ...
    """Train and evaluate every model across rolling cutoffs and seeds."""
    from mktrade.eval.backtest import make_backtest_windows
    from mktrade.train.splits import temporal_split

    windows = make_backtest_windows(
        sorted(atlas_df["year"].unique()),
        train_cfg.rolling_cutoffs,
        persistence_years=train_cfg.min_consecutive_years,
    )
    rows: list[dict[str, float | int | str]] = []
    checkpoint_dir = Path("models") / "backtests"
    for window in windows:
        for seed in train_cfg.evaluation_seeds:
            fold_cfg = train_cfg.model_copy(deep=True)
            fold_cfg.train_end_year = window.train_end_year
            fold_cfg.val_year = window.validation_year
            fold_cfg.test_years = list(window.test_years)
            fold_cfg.seed = int(seed)
            np.random.seed(seed)
            ...
            for model_name in model_names:
                logger.info(
                    f"Rolling backtest: model={model_name}, cutoff={window.train_end_year}, seed={seed}"
                )
                train_data, val_data, test_data = temporal_split(
                    ...
                    wdi_df=wdi_df,
                    cefta_members=set(data_cfg.cefta_members),
                    proximity_top_k=50 if model_name == "hgt" else None,
                )
                metrics = _train_and_eval(
                    model_name,
                    train_data,
                    ...
                    fold_cfg,
                    checkpoint_dir,
                    suffix=f"_cutoff{window.train_end_year}_seed{seed}",
                )
                if metrics is not None:
                    rows.append(
                        {
                            "model": model_name,
                            "cutoff": window.train_end_year,
                            "seed": seed,
                            **metrics,
                        }
                    )
...
def main() -> None:
    ...
    logger.info(f"{'='*60}")
    logger.info("PHASE 4 — GNN Training & Evaluation")
    logger.info(f"{'='*60}")
    ...
    for model_name in models_to_train:
        ...
        if model_name == "hgt":
            logger.info(f"  Building sparsified graph for HGT (top-{hgt_prox_top_k} proximity)")
            hgt_train, hgt_val, hgt_test = temporal_split(
                ...
                wdi_df=wdi_df,
                cefta_members=set(data_cfg.cefta_members),
                proximity_top_k=hgt_prox_top_k,
            )
    ...
    try:
        ...
        # Build the same sustained-success edge set used by the GNN.
        from mktrade.opportunities.targets import criteria_from_config, sustained_link_set

        train_exp = atlas_df[atlas_df["year"] <= train_cfg.train_end_year]
        train_edge_set = sustained_link_set(train_exp, criteria_from_config(train_cfg))
        ...
        density_at_cutoff = complexity_df.copy()
        if "year" in density_at_cutoff.columns:
            density_at_cutoff = density_at_cutoff[
                density_at_cutoff["year"] <= train_cfg.train_end_year
            ]
            density_at_cutoff = density_at_cutoff[
                density_at_cutoff["year"] == density_at_cutoff["year"].max()
            ]
        density_results = density_baseline(
            density_df=density_at_cutoff[["iso3", "hs4", "density"]].dropna(),
            train_edges=train_edge_set,
            test_edges=test_edges_list,
            test_labels=test_labels_t.numpy(),
            score_col="density",
            query_ids=test_ei[0].numpy(),
        )
        ...
        try:
            ...
            if ppml_model is not None:
                ...
                if not grav_pred.empty:
                    ...
                    gravity_scores = []
                    for c_iso, _p_hs4 in test_edges_list:
    ...
    if args.rolling_backtest:
        logger.info("\nRunning rolling-origin backtests across configured seeds...")
        rolling = _run_rolling_backtests(
            atlas_df,
            country_complexity,
            product_complexity,
            proximity_df,
        ...
        rolling_path = Path("reports") / "rolling_backtests.csv"
        rolling.to_csv(rolling_path, index=False, float_format="%.6f")

        from mktrade.eval.backtest import aggregate_backtest_metrics

        aggregate = aggregate_backtest_metrics(rolling)
        aggregate_path = Path("reports") / "rolling_backtest_summary.csv"
        aggregate.to_csv(aggregate_path, index=False, float_format="%.6f")
        logger.info(f"Rolling results saved to {rolling_path} and {aggregate_path}")
