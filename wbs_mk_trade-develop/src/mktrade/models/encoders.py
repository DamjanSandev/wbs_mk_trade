...
from torch_geometric.nn import GATConv, HGTConv, Linear, SAGEConv
...
class GATEncoder(nn.Module):
    ...
    def __init__(
        ...
        node_types: list[str],
        metadata: tuple,
        edge_dims: dict[tuple[str, str, str], int] | None = None,
        dropout: float = 0.3,
    ) -> None:
        super().__init__()
        self.dropout = dropout
            ...
            for et in metadata[1]:
                ...
                if layer_idx < num_layers - 1:
                    conv_dict[key] = GATConv(
                        ...
                        edge_dim=self.edge_dims.get(et),
                    )
                else:
                    conv_dict[key] = GATConv(
    ...
    def forward(
        ...
        edge_attr_dict: dict[tuple, torch.Tensor] | None = None,
        ...
        for layer_dict in self.convs[:-1]:
            ...
            for et, ei in edge_index_dict.items():
                ...
                if key in layer_dict:
                    ...
                    edge_attr = self._edge_attributes(et, edge_attr_dict)
                    out = layer_dict[key]((x_dict[src_type], x_dict[dst_type]), ei, edge_attr)
        ...
        for et, ei in edge_index_dict.items():
            ...
            if key in last_layer:
                src_type, _, dst_type = et
                edge_attr = self._edge_attributes(et, edge_attr_dict)
                out = last_layer[key]((x_dict[src_type], x_dict[dst_type]), ei, edge_attr)
                out_dict[dst_type].append(out)
    ...
    def _edge_attributes(
        ...
        """Stabilise raw monetary/gravity attributes before attention."""

        if edge_type not in self.edge_dims or not edge_attr_dict or edge_type not in edge_attr_dict:
            return None
        attributes = edge_attr_dict[edge_type].float()
        if attributes.ndim == 1:
            attributes = attributes.unsqueeze(1)
        attributes = torch.nan_to_num(attributes, nan=0.0, posinf=0.0, neginf=0.0)
        if attributes.numel() and attributes.abs().max() > 100:
            attributes = torch.sign(attributes) * torch.log1p(attributes.abs())
        mean = attributes.mean(dim=0, keepdim=True)
        std = attributes.std(dim=0, unbiased=False, keepdim=True).clamp_min(1e-6)
